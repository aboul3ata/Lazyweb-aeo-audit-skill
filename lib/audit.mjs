...
export async function auditSite(inputUrl, options = {}) {
  ...
  const scope = inferAuditScope(evidence, parsed, auxiliary);
  applyAuditScope(evidence, scope);
  const categories = scoreCategories(evidence, parsed, auxiliary, scope);
  const totalScore = scoreOverall(categories);
  const grade = gradeFor(totalScore);
  const recommendations = buildRecommendations(evidence, categories, scope);
...
export function renderMarkdownReport(result) {
  ...
  const surfaceRows = result.evidence.surfaces.filter((surface) => surface.applicable !== false).map((surface) => (
    `| ${surface.label} | ${surface.present ? "Present" : "Missing"} | ${surface.status || "n/a"} | ${surface.detail} |`
  )).join("\n");
  const researchLensFinalNote = result.scope?.agentMode || result.scope?.programmableAccess || result.scope?.mcp
    ? "Strong AEO combines classic SEO crawlability with machine-readable agent setup surfaces."
    : "Strong AEO combines classic SEO crawlability with clear entity identity, machine-readable context, and source pages that assistants can cite.";
...
Audit scope: ${result.scope?.summary || "General public website"}. Checks that are not relevant to this site type are excluded from scoring and recommendations.
...
function inferAuditScope(evidence, parsed, auxiliary) {
  ...
  const docsText = Object.entries(auxiliary)
    ...
    .map(([, item]) => item.body || "")
    .join("\n");
  const evidenceText = `${homepageText}\n${docsText}`;
  const linkText = homepageLinks.join("\n");

  const hasPricingIntent = hasSurface("pricingMd")
    || hasSurface("pricing")
    || /\/pricing\b/i.test(linkText)
    || /\b(pricing|paid plans?|enterprise plan|subscription tiers?|per month|per year|\$\d|free trial)\b/i.test(evidenceText);
  const hasApiIntent = hasSurface("openapi")
    || /\/(?:api|developers?|docs)\b/i.test(linkText)
    || /\b(api|openapi|developer docs?|developer portal|sdk|webhook|graphql|rest endpoint)\b/i.test(evidenceText);
  const hasMcpIntent = hasSurface("mcpServerCard")
    || hasSurface("wellKnownMcp")
    || /\b(mcp|model context protocol|webmcp|server-card|tools\/list)\b/i.test(evidenceText);
  const hasAgentProductIntent = hasMcpIntent
    || hasApiIntent
    || hasSurface("agentJson")
    || hasSurface("agentCard")
    || hasSurface("aiPlugin")
    || /\b(ai agents?|agents can|agent[- ]readable|agent[- ]friendly|codex|claude|cursor|chatgpt|plugin|skill)\b/i.test(evidenceText);
  const hasDeepDocsIntent = hasSurface("llmsFull")
    || hasApiIntent
    || hasMcpIntent
    || docsText.length > 3000
    || homepageLinks.length >= 30
    || /\b(documentation|developer docs?|knowledge base|reference|guide|tutorials?)\b/i.test(evidenceText);
  const hasCommercialProductIntent = hasPricingIntent
    || hasApiIntent
    || hasMcpIntent
    || /\b(software|platform|service|tool|customers?|teams?|enterprise|demo|trial|buy)\b/i.test(evidenceText)
    || /\b(product|app)\s+(?:for|that|to)\b/i.test(evidenceText);
  const hasContentPublisherIntent = /\b(newsletter|blog|article|archive|essay|publication|subscribe|author|posts?)\b/i.test(evidenceText)
    && !hasApiIntent
    && !hasMcpIntent;
  const primaryContentPublisher = hasContentPublisherIntent && !hasPricingIntent && !hasAgentProductIntent;
  const commercialScope = hasCommercialProductIntent && !primaryContentPublisher;

  let summary = "General public website";
  if (hasAgentProductIntent) summary = "Agent-facing product or programmable service";
  else if (primaryContentPublisher) summary = "Content, newsletter, or publication site";
  else if (commercialScope) summary = "Commercial product or service";

  return {
    summary,
    pricing: hasPricingIntent,
    longContextDocs: hasDeepDocsIntent,
    agentMode: hasAgentProductIntent,
    wellKnownAgentDiscovery: hasAgentProductIntent,
    linkHeaders: hasAgentProductIntent || hasDeepDocsIntent || hasSurface("llms") || hasSurface("pricingMd") || hasSurface("openapi"),
    programmableAccess: hasApiIntent,
    mcp: hasMcpIntent,
    installSetup: hasAgentProductIntent || hasApiIntent,
    responseSchemas: hasApiIntent || hasMcpIntent,
    productStructuredData: commercialScope || hasAgentProductIntent,
    useCaseContent: commercialScope || hasAgentProductIntent || parsed.hasUseCaseText || parsed.hasComparisonText,
    faqTables: commercialScope || hasAgentProductIntent || parsed.hasFaqText || parsed.questions > 0 || parsed.tableCount > 0,
    contentPublisher: primaryContentPublisher
  };
}

function applyAuditScope(evidence, scope) {
  const applicableBySurface = {
    llms: true,
    llmsFull: scope.longContextDocs,
    pricingMd: scope.pricing,
    pricing: scope.pricing,
    openapi: scope.programmableAccess,
    robots: true,
    sitemap: true,
    agentJson: scope.wellKnownAgentDiscovery,
    aiPlugin: scope.wellKnownAgentDiscovery,
    agentCard: scope.wellKnownAgentDiscovery,
    mcpServerCard: scope.mcp,
    wellKnownMcp: scope.mcp,
    agentMode: scope.agentMode
  };

  evidence.surfaces = evidence.surfaces.map((surface) => ({
    ...surface,
    applicable: applicableBySurface[surface.key] !== false
  }));
}

function scoreCategories(evidence, parsed, auxiliary, scope) {
  const hasSurface = (key) => evidence.surfaces.find((surface) => surface.key === key)?.present;
  ...
  const explanationPositive = scope.contentPublisher ? "homepage explains the publication and audience" : "homepage explains what the product does";
  const explanationNegative = scope.contentPublisher ? "publication explanation is too thin" : "product explanation is too thin";
  const discoveryPositive = scope.contentPublisher ? "topic or archive discovery content is present" : "use-case or comparison content is present";
  const discoveryNegative = scope.contentPublisher ? "topic or archive discovery content is weak" : "non-brand discovery content is weak";
  ...
  return {
    discovery: category("Discovery and machine-readable docs", CATEGORY_MAX.discovery, [
      ...
      points(hasSurface("llmsFull"), 2, "llms-full.txt is present", "llms-full.txt is missing", { applicable: scope.longContextDocs }),
      points(hasSurface("pricingMd") || hasSurface("pricing"), hasSurface("pricingMd") ? 3 : 1, "pricing is discoverable", "pricing.md and pricing page are missing", { applicable: scope.pricing }),
      ...
      points(agentDocLinkHeader, 2, "HTTP Link headers advertise machine-readable docs", "HTTP Link headers do not advertise machine-readable docs", { applicable: scope.linkHeaders }),
      points(hasSurface("agentMode"), 2, "agent mode is available", "?mode=agent is missing", { applicable: scope.agentMode }),
      points(wellKnownCount >= 2, 4, `${wellKnownCount} well-known agent surfaces found`, "well-known agent discovery files are missing", { applicable: scope.wellKnownAgentDiscovery })
    ]),
    ...
    entityTrust: category("Entity, trust, and structured data", CATEGORY_MAX.entityTrust, [
      ...
      points(homepageUsable && hasBroadSchema, 4, "schema type breadth exists", "schema breadth is too narrow", { applicable: scope.productStructuredData }),
    ...
    answerability: category("Answerability and citation readiness", CATEGORY_MAX.answerability, [
      points(homepageUsable && /\b(what|help|platform|product|software|service|newsletter|publication|archive|essays?|articles?|for)\b/i.test(homepageText) && parsed.wordCount >= 300, 4, explanationPositive, explanationNegative),
      points(homepageUsable && parsed.hasNumbers, 3, "concrete numerical claims are present", "quotable numerical claims are missing"),
      points(homepageUsable && (parsed.hasUseCaseText || parsed.hasComparisonText || (scope.contentPublisher && /\b(archive|topics?|essays?|articles?|posts?|newsletter)\b/i.test(homepageText))), 4, discoveryPositive, discoveryNegative, { applicable: scope.useCaseContent || scope.contentPublisher }),
      points((homepageUsable && parsed.hasPricingText) || hasSurface("pricing") || hasSurface("pricingMd"), 3, "pricing or plan language is extractable", "pricing is not extractable", { applicable: scope.pricing }),
      points(homepageUsable && (parsed.hasFaqText || parsed.questions >= 4 || parsed.tableCount > 0), 3, "FAQ/questions/tables support citations", "FAQ/questions/tables are missing", { applicable: scope.faqTables }),
    ...
    integration: category("Agent integration and programmable access", CATEGORY_MAX.integration, [
      points(hasDeveloperDocs, 4, "developer/API docs are discoverable", "developer/API docs are missing", { applicable: scope.programmableAccess }),
      points(hasAuthDocs, 4, "auth/rate-limit/error guidance is detectable", "auth, scopes, rate limits, or retry guidance are missing", { applicable: scope.programmableAccess }),
      points(hasMcpDocs, 4, "MCP/WebMCP discovery is detectable", "MCP/WebMCP discovery is missing", { applicable: scope.mcp }),
      points(/\b(sdk|cli|install|cursor|codex|claude|chatgpt|plugin|skill)\b/i.test(evidenceText), 3, "install/SDK/CLI/platform setup is detectable", "install/SDK/CLI/platform setup is missing", { applicable: scope.installSetup }),
      points(hasSurface("openapi") || /"schema"|"operationId"|response schema|pagination/i.test(docsText), 2, "schemas/examples are discoverable", "typed response schemas are missing", { applicable: scope.responseSchemas }),
      points(hasSurface("agentMode") || hasSurface("llms") || hasSurface("pricingMd"), 3, "agent can gather setup context without UI clicks", "agent setup still depends on marketing UI", { applicable: scope.agentMode || scope.longContextDocs })
    ])
  };
}
...
function category(label, max, checks) {
  ...
  const earned = applicableChecks.reduce((sum, check) => sum + check.score, 0);
  const applicable = possible > 0;
  const score = applicable ? Math.min(max, Math.round((earned / possible) * max)) : null;
  return {
    label,
    score,
    max,
    applicable,
    checks,
    notes: applicableChecks.map((check) => check.note),
    omittedChecks: checks.filter((check) => !check.applicable).map((check) => check.note)
  };
}

function points(condition, score, positive, negative, options = {}) {
  const applicable = options.applicable !== false;
  return {
    applicable,
    passed: Boolean(condition),
    score: applicable && condition ? score : 0,
    weight: score,
    note: condition ? positive : negative
  };
}

function scoreOverall(categories) {
  const applicableCategories = Object.values(categories).filter((categoryItem) => categoryItem.applicable);
  const earned = applicableCategories.reduce((sum, categoryItem) => sum + categoryItem.score, 0);
  const possible = applicableCategories.reduce((sum, categoryItem) => sum + categoryItem.max, 0);
  return possible ? Math.round((earned / possible) * 100) : 0;
}

function buildRecommendations(evidence, categories, scope) {
  const recs = [];
  const surface = (key) => evidence.surfaces.find((item) => item.key === key);
  const missing = (key) => !surface(key)?.present;
  const llmsAction = scope.agentMode || scope.programmableAccess || scope.mcp || scope.pricing
    ? "Create /llms.txt with overview, use cases, pricing/docs links, API or tool instructions where applicable, constraints, and support contacts."
    : "Create /llms.txt with a concise overview, audience, key topics, archive or sitemap links, subscription path, update cadence, and contact details.";
  const rawHtmlTitle = scope.contentPublisher ? "Make raw HTML explain the publication" : "Make raw HTML answer what the product does";
  const rawHtmlAction = scope.contentPublisher
    ? "Server-render or prerender the publication premise, author/editor context, topic areas, archive links, subscription path, and representative recent posts."
    : "Server-render or prerender a clear product explanation, use cases, differentiation, pricing/next step, and FAQs.";
  ...
  addIf(recs, missing("llms"), "Publish a substantive /llms.txt", "P0", "Discovery", "No usable /llms.txt was fetched.", llmsAction, "curl -i https://domain.com/llms.txt and confirm 20+ useful lines.");
  addIf(recs, scope.pricing && missing("pricingMd"), "Add /pricing.md for agent plan comparison", "P0", "Discovery", "Machine-readable pricing.md is missing.", "Publish plan names, prices, limits, feature gates, trial terms, enterprise notes, and last-updated date in Markdown.", "curl https://domain.com/pricing.md and verify agents can compare tiers without rendering HTML.");
  addIf(recs, evidence.homepage.wordCount < 500, rawHtmlTitle, "P0", "Crawlability", `Only ${evidence.homepage.wordCount} visible words were found in raw HTML.`, rawHtmlAction, "curl the homepage and verify the text is present before JavaScript executes.");
  addIf(recs, scope.agentMode && missing("agentMode"), "Add a dedicated ?mode=agent view", "P1", "Discovery", "?mode=agent did not return a substantive agent view.", "Return compact machine-readable product facts, endpoints, auth, pricing, and setup paths when mode=agent is requested.", "curl 'https://domain.com/?mode=agent' and confirm it differs from marketing HTML.");
  ...
  addIf(recs, scope.programmableAccess && missing("openapi"), "Publish OpenAPI or explain no public API", "P1", "Agent integration", "No /openapi.json was fetched.", "Expose OpenAPI for programmable endpoints or document that the product is UI-only.", "curl https://domain.com/openapi.json and validate operationIds and schemas.");
  addIf(recs, scope.mcp && missing("mcpServerCard") && missing("wellKnownMcp"), "Add MCP/WebMCP discovery when agents should use tools", "P1", "Agent integration", "No MCP well-known or server card was fetched.", "Publish /.well-known/mcp/server-card.json and a reachable Streamable HTTP MCP endpoint when applicable.", "Initialize MCP with a clean client and list tools.");
